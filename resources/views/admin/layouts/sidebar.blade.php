<div @click.stop="closeSidebar()"
    class="fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity z-30 md:hidden cursor-pointer" :class="{ 
        'opacity-100 visible pointer-events-auto': sidebarOpen,
        'opacity-0 invisible pointer-events-none': !sidebarOpen
    }" x-transition:enter="transition-opacity ease-linear duration-300" x-transition:enter-start="opacity-0"
    x-transition:enter-end="opacity-100" x-transition:leave="transition-opacity ease-linear duration-300"
    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
</div><!-- Sidebar (Fixed on Mobile & Tablet, Static on Desktop) -->
<aside
    class="fixed lg:static inset-y-0 left-0 bg-gradient-to-br from-[#134686] via-[#1a5fa0] to-[#1a5fa0] text-gray-50 w-64 flex-shrink-0 overflow-y-auto shadow-2xl z-40 lg:shadow-none transform transition-transform duration-300 ease-in-out"
    :class="{
    ...
    <nav class="p-4">
        <ul class="space-y-2">
            ...
            <!-- Divider -->
            <li class="pt-4 pb-2">
                <span class="px-4 text-xs font-semibold text-white/70 uppercase tracking-wider">Tampilan</span>
            </li>

            <!-- Hero Section -->
            <li>
                <a href="{{ route('admin.hero.index') }}"
                    class="flex items-center px-4 py-3 rounded-xl transition duration-200 {{ request()->routeIs('admin.hero.*') ? 'bg-white/25 backdrop-blur-sm text-white shadow-lg font-semibold' : 'hover:bg-white/10' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
