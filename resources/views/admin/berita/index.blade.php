...
<div x-data="{ deleteModalOpen: false, deleteId: null }" class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-primary-600 to-primary-700 rounded-2xl shadow-lg p-8 text-white">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold font-montserrat mb-2">Menu Berita</h1>
                <p class="text-primary-100">Kelola semua data berita dan kegiatan Karang Taruna</p>
            </div>
            <div class="bg-white/20 backdrop-blur-sm rounded-xl p-4">
                <svg class="w-10 h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v4m0 0a2 2 0 002 2h2a2 2 0 002-2m0-4a2 2 0 012 2v4a2 2 0 01-2 2h-2.5"></path>
                </svg>
            </div>
        </div>
    </div>
    ...
    <div class="flex justify-between items-center">
        <div>
            <p class="text-gray-600 text-sm">Total berita: <span class="font-bold text-primary-600">{{ $berita->total() }}</span></p>
        </div>
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-sm font-semibold text-gray-700">Thumbnail</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($berita as $index => $item)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $berita->firstItem() + $index }}</td>
                            <td class="px-6 py-4 text-sm">
                                @if($item->thumbnail)
                                    <img src="{{ asset('storage/' . $item->thumbnail) }}" alt="{{ $item->judul }}" class="h-12 w-12 object-cover rounded-lg">
                                @else
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-gray-500 text-lg font-semibold mb-2">Belum ada data berita</p>
                                    <p class="text-gray-400 text-sm mb-4">Silakan tambahkan berita baru untuk memulai</p>
                                    <a href="{{ route('admin.berita.create') }}"
    ...
    <div x-show="deleteModalOpen"
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 space-y-6 animate-in fade-in zoom-in duration-300">
            ...
            <div class="text-center space-y-2">
                <h3 class="text-xl font-bold text-gray-800">Hapus Berita?</h3>
                <p class="text-gray-600 text-sm">Anda yakin ingin menghapus berita ini? Tindakan ini tidak dapat dibatalkan.</p>
            </div>
            ...
            <div class="flex gap-3">
                ...
                <form :action="`{{ route('admin.berita.destroy', '') }}/${deleteId}`" method="POST" class="flex-1">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
