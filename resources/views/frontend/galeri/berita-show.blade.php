                ...
                <div class="flex items-center space-x-6 flex-wrap gap-4">
                    ...
                    <div class="flex items-center space-x-3 bg-white/10 backdrop-blur-sm px-4 py-3 rounded-xl">
                        ...
                        <div>
                            ...
                            <div class="font-semibold">{{ ceil(str_word_count(strip_tags($kegiatan->isi ?? $kegiatan->deskripsi)) / 200) }} min</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="max-w-6xl mx-auto">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    @if($kegiatan->media_path && is_array($kegiatan->media_path) && count($kegiatan->media_path) > 0)
                        <div class="bg-gradient-to-br from-white via-blue-50 to-white rounded-2xl shadow-lg p-8 border border-blue-100 mb-8 overflow-hidden">
                            ...
                            <div class="relative z-10">
                                <h2 class="text-2xl font-bold text-gray-800 mb-6 flex items-center">
                                    ...
                                    <span>Galeri Foto <span class="text-blue-600 font-bold ml-2 bg-blue-100 px-3 py-1 rounded-full text-lg">{{ count($kegiatan->media_path) }}</span></span>
                                </h2>
                                    ...
                                    @foreach($kegiatan->media_path as $index => $media)
                                        <div class="group relative aspect-square overflow-hidden rounded-xl border-2 border-blue-200 hover:border-blue-500 transition-all duration-300 cursor-pointer shadow-md hover:shadow-lg transform hover:scale-105"
                                            ...
                                            <img src="{{ asset('storage/' . $media) }}" 
                                                 ...
                                                 loading="lazy">
                                            <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-black/0 to-black/0 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center">
                                                <svg class="w-12 h-12 text-white mb-2 group-hover:scale-125 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7"></path>
                                                </svg>
                                                <p class="text-white text-xs font-semibold">Preview</p>
                                            </div>
                                            <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                                <p class="text-white text-xs font-semibold">Foto {{ $index + 1 }}/{{ count($kegiatan->media_path) }}</p>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        <div id="lightbox" class="fixed inset-0 bg-black/95 backdrop-blur-sm z-50 hidden items-center justify-center p-4" onclick="closeLightbox(event)">
                            <!-- Close Button -->
                            <button onclick="closeLightbox(event)" class="absolute top-4 right-4 z-50 bg-white/10 hover:bg-white/20 text-white p-2 rounded-lg transition-all duration-200 group">
                            ...
                            <!-- Previous Button -->
                            <button onclick="previousImage(event)" class="absolute left-4 top-1/2 transform -translate-y-1/2 z-50 bg-white/10 hover:bg-white/20 text-white p-3 rounded-lg transition-all duration-200 group hidden sm:block">
                                <svg class="w-8 h-8 group-hover:scale-125 transition-transform group-hover:-translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="flex flex-col items-center justify-center max-w-4xl w-full">
                                <img id="lightbox-image" src="" alt="Preview" class="max-w-full max-h-[70vh] object-contain rounded-lg shadow-2xl border-4 border-white/20">
                                
                                <!-- Mobile Navigation Buttons -->
                                <div class="flex gap-3 mt-6 sm:hidden w-full justify-center">
                                    <button onclick="previousImage(event)" class="flex-1 bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-lg transition-all flex items-center justify-center gap-2">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                                        </svg>
                                        <span>Prev</span>
                                    </button>
                                    <button onclick="nextImage(event)" class="flex-1 bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-lg transition-all flex items-center justify-center gap-2">
                                        <span>Next</span>
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                                        </svg>
                            ...
                            <!-- Next Button -->
                            <button onclick="nextImage(event)" class="absolute right-4 top-1/2 transform -translate-y-1/2 z-50 bg-white/10 hover:bg-white/20 text-white p-3 rounded-lg transition-all duration-200 group hidden sm:block">
                                <svg class="w-8 h-8 group-hover:scale-125 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                                </svg>
                            </button>
                            ...
                            <div class="absolute bottom-4 left-0 right-0 text-center">
                                <p id="lightbox-caption" class="text-lg font-semibold text-white bg-black/50 backdrop-blur-sm py-2 px-4 rounded-full inline-block"></p>
                            </div>
                        </div>
                    @endif
                    ...
                    @if($kegiatan->link_video)
                        <div class="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl shadow-xl p-8 border border-slate-700 mb-8 overflow-hidden">
                            <!-- Decorative gradient overlay -->
                            <div class="absolute inset-0 bg-gradient-to-r from-primary-600/10 to-accent/10 opacity-50"></div>
                            
                            <div class="relative z-10">
                                <h2 class="text-2xl font-bold text-white mb-6 flex items-center">
                                    <div class="bg-accent/20 backdrop-blur-sm p-2 rounded-lg mr-3 border border-accent/30">
                                        <svg class="w-6 h-6 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"></path>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                        </svg>
                                    </div>
                                    <span class="text-transparent bg-clip-text bg-gradient-to-r from-accent to-yellow-300">Video Kegiatan</span>
                                </h2>
                                
                                <div class="relative aspect-video bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl overflow-hidden border-2 border-accent/30 shadow-2xl">
                                    @php
                                ...
                                <div class="mt-6 flex items-center justify-between flex-wrap gap-4">
                                    <p class="text-slate-300 text-sm font-medium">
                                        🎬 Video dari Kegiatan: <span class="text-accent font-semibold">{{ $kegiatan->judul }}</span>
                                    </p>
    ...
    <script>
        ...
        function showImage() {
            if (images.length > 0) {
                ...
                document.getElementById('lightbox-caption').textContent = `Foto ${currentImageIndex + 1} dari ${images.length}`;
            }
        }
