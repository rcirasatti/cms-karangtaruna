...
@section('content')
    ...
    <div class="bg-gray-100 pt-6 pb-16 md:py-16">
        <div class="container mx-auto px-4">
            ...
            <!-- Modal Detail Produk -->
            <div id="produkModal"
                class="hidden fixed inset-0 bg-transparent z-50 flex items-center justify-center p-2 sm:p-4"
                onclick="closeProdukModal(event)">
                <div class="bg-white rounded-xl sm:rounded-2xl max-w-4xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-y-auto shadow-2xl"
                    onclick="event.stopPropagation()">

                    <!-- Header Modal -->
                    <div class="sticky top-0 z-10 bg-gradient-to-r from-primary-600 to-primary-800 text-white p-3 sm:p-4 md:p-6 rounded-t-xl sm:rounded-t-2xl">
                        <button onclick="closeProdukModal()"
                            class="absolute top-2 right-2 sm:top-3 sm:right-3 md:top-4 md:right-4 text-white hover:text-gray-200 transition-colors z-20">
                        ...
                        <div class="pr-8 sm:pr-10">
                            <h3 id="modalNamaProduk" class="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-yellow-400 uppercase mb-1 leading-tight"></h3>
                            <p id="modalKategoriProduk" class="text-blue-100 text-xs sm:text-sm font-medium"></p>
                        </div>
                    </div>
                    ...
                    <div class="p-3 sm:p-4 md:p-6">
                        ...
                        <div id="modalGaleriSection" class="mb-4 sm:mb-6">
                            <div class="relative bg-gray-100 rounded-lg sm:rounded-xl overflow-hidden">
                                <!-- Main Image -->
                                <div id="modalMainImageContainer" class="relative h-48 sm:h-64 md:h-80 lg:h-96 bg-gradient-to-br from-gray-200 to-gray-300">
                                    <img id="modalMainImage" src="" alt="" class="w-full h-full object-contain">
                                    
                                    <!-- Navigation Arrows for Modal Gallery -->
                                    <button id="modalPrevImage" class="hidden absolute left-1 sm:left-2 md:left-4 top-1/2 transform -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-1.5 sm:p-2 md:p-3 rounded-full transition-all">
                                        <svg class="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                                        </svg>
                                    </button>
                                    <button id="modalNextImage" class="hidden absolute right-1 sm:right-2 md:right-4 top-1/2 transform -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-1.5 sm:p-2 md:p-3 rounded-full transition-all">
                                        <svg class="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                                        </svg>
                                    </button>
                                ...
                                <!-- Thumbnail Gallery -->
                                <div id="modalThumbnails" class="hidden flex gap-1.5 sm:gap-2 mt-2 sm:mt-3 md:mt-4 overflow-x-auto pb-2 px-1">
                        ...
                        <!-- Harga Produk -->
                        <div class="mb-4 sm:mb-6 bg-primary-50 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-primary-100">
                            <h4 class="text-sm sm:text-base md:text-lg font-bold text-gray-800 mb-1.5 sm:mb-2 flex items-center">
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-1.5 sm:mr-2 text-primary-600 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1.41 16.09V20h-2.67v-1.93c-1.71-.36-3.16-1.46-3.27-3.4h1.96c.1 1.05.82 1.87 2.65 1.87 1.96 0 2.4-.98 2.4-1.59 0-.83-.44-1.61-2.67-2.14-2.48-.6-4.18-1.62-4.18-3.67 0-1.72 1.39-2.84 3.11-3.21V4h2.67v1.95c1.86.45 2.79 1.86 2.85 3.39H14.3c-.05-1.11-.64-1.87-2.22-1.87-1.5 0-2.4.68-2.4 1.64 0 .84.65 1.39 2.67 1.91s4.18 1.39 4.18 3.91c-.01 1.83-1.38 2.83-3.12 3.16z"/>
                                </svg>
                                <span class="truncate">Harga Produk</span>
                            </h4>
                            <p id="modalHargaProduk" class="text-xl sm:text-2xl md:text-3xl font-bold text-primary-600 break-words"></p>
                        </div>

                        <!-- Deskripsi Produk -->
                        <div class="mb-4 sm:mb-6">
                            <h4 class="text-sm sm:text-base md:text-lg font-bold text-gray-800 mb-2 sm:mb-3 flex items-center">
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-1.5 sm:mr-2 text-primary-600 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                                ...
                                <span class="truncate">Deskripsi Produk</span>
                            </h4>
                            <p id="modalDeskripsiProduk" class="text-xs sm:text-sm md:text-base text-gray-700 leading-relaxed whitespace-pre-line"></p>
                        </div>
                    </div>

                    <!-- Footer Modal - Sticky di bottom -->
                    <div class="sticky bottom-0 bg-gray-50 px-3 sm:px-4 md:px-6 py-3 sm:py-4 rounded-b-xl sm:rounded-b-2xl flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-2 sm:gap-3 md:gap-4 border-t border-gray-200">
                        <button onclick="closeProdukModal()"
                            class="order-2 sm:order-1 px-4 sm:px-5 md:px-6 py-2 sm:py-2.5 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition-colors font-medium text-sm sm:text-base">
                            Tutup
                        </button>
                        <button id="modalPesanWhatsApp"
                            class="order-1 sm:order-2 px-4 sm:px-5 md:px-6 py-2 sm:py-2.5 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white rounded-lg transition-colors font-medium flex items-center justify-center space-x-2 text-sm sm:text-base">
...
@push('scripts')
    <script>
            ...
            if (currentModalImages.length > 0) {
                ...
                if (currentModalImages.length > 1) {
                    ...
                    currentModalImages.forEach((img, index) => {
                        ...
                        thumb.className = 'flex-shrink-0 w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 rounded-md sm:rounded-lg overflow-hidden border-2 transition-all ' + 
        ...
        // Touch swipe for mobile image navigation
        let touchStartX = 0;
        let touchEndX = 0;

        const modalImageContainer = document.getElementById('modalMainImageContainer');
        ...
        function handleSwipe() {
            const modal = document.getElementById('produkModal');
            if (modal.classList.contains('hidden') || currentModalImages.length <= 1) return;
